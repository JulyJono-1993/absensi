...
export default function PrintPage() {
  ...
  const fetchReport = async () => {
    ...
    try {
      const json = await getPrintData(selectedClassId, period, date, month);
      setData(json);
    } catch (err) {
      setToast({ show: true, message: "Gagal Memuat", description: err instanceof Error ? err.message : "Terjadi kesalahan.", type: "error" });
    }
    ...
    <div>
      ...
      {isDailyWeekend ? (
        ...
        <div id="print-area" className="bg-white rounded-2xl border border-outline-variant shadow-sm overflow-hidden">
          ...
          {/* Semester / Monthly Summary Table */}
          {(data.type === "semester" || data.type === "monthly") && (
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-surface-container-low border-b border-outline-variant">
