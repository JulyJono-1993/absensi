...
export default function AttendancePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setToast({ show: true, message: "Gagal Menyimpan", description: err instanceof Error ? err.message : "Terjadi kesalahan.", type: "error" });
    }
  ...
  const handleAutoAlpa = async () => {
    ...
    try {
      const data = await autoAlpa(selectedClassId, selectedDate);
      setToast({
        show: true,
  ...
  const handleSendWA = async () => {
    if (!selectedClassId) return;
    try {
      const data = await getReport(selectedClassId, selectedDate);
      if (data.waUrl) {
